...
const validateConfig = (config) => {
	...
	let configUndefined = _validateObjectAndUndefined(config, messages.errors.config.invalidFormat);
...
const _validateKnownFields = (config) => {
	let r = Object.keys(config).filter(item => !configDefault[item]);
...
const _validateReservedFields = (config) => {
	let reservedWords = ['environments', 'predefined'];
