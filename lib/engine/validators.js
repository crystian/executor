 ...
 * @param extraMessage a string with extraMessage to concatenate
 */
const validateShortcut = (shortcut, extraMessage) => {
	if (!(shortcut && shortcut !== '' && isString(shortcut))) {
		extraMessage = extraMessage || '';
		throw new Error(messages.shortcut.withoutArguments + extraMessage);
	}
};
...
const validateConfig = (config) => {
	...
	_validateObjectAndUndefined(config, messages.config.invalidFormat);
};
...
const validateTemplates = (templates) => {
	let r = _validateObjectAndUndefined(templates, messages.templates.invalidData);
	if (r) {
		return;
	}
