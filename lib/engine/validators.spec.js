...
const { validateEnvironments, validateConfig, validateTemplates, validateShortcuts, validateShortcut, validateConfigFileField } = require('./validators');
...
describe('validateShortcut() throws', function() {
	...
	it('should throw an error by empty arguments and show shortcuts', function() {
		...
		expect(f).to.throw(ExecutorError).has.property('code', code);
		expect(f).to.throw(ExecutorError).has.property('message').include('shortcuts1');
	});
